...
// lambda/language-snapshotter/index.ts
var index_exports = {};
...
var handler2 = async (event) => {
  console.log("Language Snapshotter received event:", JSON.stringify(event, null, 2));
  try {
    ...
    console.log(`Processing push to ${repository.full_name}, fetching language data...`);
    const { Octokit: Octokit3 } = await Promise.resolve().then(() => (init_dist_src5(), dist_src_exports));
    const octokit = new Octokit3({ auth: process.env.GITHUB_TOKEN });
    ...
    await ddbDocClient.send(new import_lib_dynamodb.PutCommand(params));
    console.log(`\u2705 Successfully saved language snapshot for ${repository.full_name}`);
    return {
      statusCode: 200,
      body: JSON.stringify({ message: "Language snapshot saved successfully" })
    };
  } catch (error) {
    console.error("Error in Language Snapshotter:", error);
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "Failed to process language snapshot" })
    };
  }
};

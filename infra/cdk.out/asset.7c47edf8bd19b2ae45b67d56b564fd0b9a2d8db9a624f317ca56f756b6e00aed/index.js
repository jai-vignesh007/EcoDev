...
var handler2 = async (event) => {
  console.log("\u{1F50D} [DEBUG] Language Snapshotter received event");
  try {
    const payload = JSON.parse(event.body);
    const { repository, ref } = payload;
    console.log(`\u{1F50D} [DEBUG] Processing push to: ${repository.full_name}, ref: ${ref}`);
    const defaultBranchRef = `refs/heads/${repository.default_branch}`;
    if (ref !== defaultBranchRef) {
      console.log(`\u{1F50D} [DEBUG] Skipping non-default branch push: ${ref}`);
      return { statusCode: 200, body: JSON.stringify({ message: "Skipped non-default branch" }) };
    }
    console.log(`\u{1F50D} [DEBUG] Fetching language data for ${repository.full_name}`);
    ...
    console.log(`\u{1F50D} [DEBUG] Writing to DynamoDB: ${process.env.LANGUAGE_TABLE_NAME}`);
